...
fn reconstruct_glyf(data: &[u8]) -> Option<(Vec<u8>, Vec<u8>)> {
    ...
    let bbox_bitmap_bytes = num_glyphs.div_ceil(8);
    ...
    for gid in 0..num_glyphs {
        ...
        if number_of_contours < 0 {
            ...
            let start = glyf.len();
            glyf.extend_from_slice(&number_of_contours.to_be_bytes());
            // bbox
            ...
            loop {
                ...
                let arg_bytes = if flags_word & ARG_1_AND_2_ARE_WORDS != 0 {
                    4
                } else {
                    2
                };
...
fn decode_simple_glyph(
    ...
    // X coordinates.
    for &dx in xs.iter().take(total_points) {
        if dx == 0 {
            // nothing (X_SAME)
        } else if (-255..=255).contains(&dx) {
    ...
    // Y coordinates.
    for &dy in ys.iter().take(total_points) {
        if dy == 0 {
        } else if (-255..=255).contains(&dy) {
