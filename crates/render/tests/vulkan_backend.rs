//! Real-Vulkan verification of the Vulkan backend.
//!
//! Every test opens a device with the Khronos validation layer (when it is
//! installed) and asserts it reported no error, so each one checks the frozen
//! SPIR-V against the shared pipeline layout, the layout transitions and the
//! submission synchronization as well as the pixels it reads back.
//!
//! Built where Vulkan is the native backend (Linux, Android) or with the
//! `vulkan` feature (MoltenVK on macOS). Skipped when no Vulkan device exists.
...
use viso_gpu::{
    ...
    TextureFormat, TextureId, VulkanBackend,
};
...
/// A validated Vulkan device, or `None` (the test is skipped) without one.
fn device() -> Option<VulkanBackend> {
    let gpu = VulkanBackend::try_new_with(true);
    if gpu.is_none() {
...
fn assert_no_validation_errors(gpu: &VulkanBackend) {
    if !gpu.validation_enabled() {
        eprintln!("Vulkan validation layer not installed; synchronization unchecked");
    }
    assert_eq!(
...
fn pipeline(
    ...
    gpu.create_pipeline(
        &PipelineDesc {
            ...
            code: entry.code(ShaderLang::SpirV),
...
/// frozen SPIR-V, and a frame's upload path runs, with no validation error.
#[test]
fn renderer_prewarms_every_standard_pipeline_on_vulkan() {
    let Some(mut gpu) = device() else { return };
