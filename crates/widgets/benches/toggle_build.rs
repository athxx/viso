//! Microbench skeleton for the `Toggle` interactive control: the cost of one
//! authoring pass (`Toggle::build`), one `layout`, and one `paint_tree`
//! lowering.
//!
//! This is the section 71 microbench slot for the third Tier 2 (interactive)
//! control — a switch — and copies the `checkbox_build.rs` template. It is
...
//! first).
//!
//! Like `CheckBox`, `Toggle` authors a reactive `on` state cell (`cx.state`) and
//! wires an `on -> PAINT` binding, so it must build through
...
//! a touch larger than a checkbox's (a track flex holding a thumb leaf, plus the
//! caption). It drives only `viso-ui`, so it needs no facade or render
...
/// the node store so the built toggle's binding/state references stay valid.
struct Reactive {
    states: StateStore,
...
/// Author a single `Toggle` (with an `on_change`) into a fresh store and return
...
fn build_scene() -> (NodeStore, viso_ui::NodeId) {
    ...
    let root = {
        ...
        toggle("Wi-Fi").on_change(|_, _| {}).build(&mut cx);
...
fn bench_toggle(c: &mut Criterion) {
    // build: author the Toggle into a fresh NodeStore through a reactive cx.
    c.bench_function("toggle/build", |b| {
        b.iter(|| {
            let mut store = NodeStore::new();
            let mut r = Reactive::new();
            let mut cx =
    ...
    // layout: lay the built toggle out into the surface rect.
    c.bench_function("toggle/layout", |b| {
    ...
    // paint_tree: lower the laid-out toggle into the reused primitive buffer.
    c.bench_function("toggle/paint_tree", |b| {
        let (mut store, root) = build_scene();
...
criterion_group!(benches, bench_toggle);
