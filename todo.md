...
- [ ] **macOS 自绘窗口外框(混合外框)** —— 照 makepad `platform/src/os/apple/macos/*` 语义(取语义不取架构,
      ...
      **本 phase 只打通 macOS 平台机制 + 暴露契约,不做 viso-ui caption widget**(那属 widgets 层,后续 phase)。
      分层:platform 只做 style mask / 去装饰 / mouseDown 夺回 / 红绿灯测量;caption 内容由 viso-ui 画(§3.5/§23)。
  ...
  **后续 phase(本次不做):** Windows 自绘外框(WM_NCCALCSIZE + DwmExtendFrameIntoClientArea + 自绘 min/max/close);
  Linux CSD + xdg-decoration 协商 + 自绘按钮;viso-ui/widgets caption bar widget(标题/图标/非 macOS 自绘红绿灯,跨三家统一);
...
- [x] **TF-P0.3 basic shaping(rustybuzz 单 run)** —— shaping.rs Latin LTR 基线。**验收:** Latin run shape golden(advance/glyph id)。
- [x] **TF-P0.4 A8 Coverage atlas page cache** —— raster_a8.rs A8 exact-coverage 光栅 + glyph_cache.rs A8 pool page-level CLOCK residency 元数据(render 拥有实际纹理)。
...
- [x] **TF-P5.2 packaged font lazy fetch + External FontProvider + progressive subset** —— font_provider.rs + progressive.rs;world-ready paragraph 语义与 Native 共享。**验收:** lazy fetch/decode + 渐进子集测。
...
- [x] **TF-P6.3 very-large paragraph 增量正确性/性能** —— 超大段增量。**验收:** 大段增量正确性对拍 + 增量性能 microbench。
